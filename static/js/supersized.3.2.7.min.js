    ...
    a.supersized = function (b) {
        ...
        d._build = function () {
            ...
            while (g <= d.options.slides.length - 1) {
                ...
                if (g == d.options.start_slide - 1) {
                    ...
                    if (d.options.thumb_links) {
                        d.options.slides[g].thumb ? i = d.options.slides[g].thumb : i = d.options.slides[g].img;
            ...
            if (d.options.thumbnail_navigation) {
                ...
                a(vars.prev_thumb).show().html(a("<img/>").attr("src", d.options.slides[prevThumb].img));
                vars.current_slide == d.options.slides.length - 1 ? nextThumb = 0 : nextThumb = vars.current_slide + 1;
                a(vars.next_thumb).show().html(a("<img/>").attr("src", d.options.slides[nextThumb].img))
            }
            d._start()
        };
        d._start = function () {
            ...
            if (d.options.slides.length > 1) {
                if (d.options.slides.length > 2) {
                    ...
                    var q = a('<img src="' + d.options.slides[loadPrev].img + '"/>');
                    var n = d.el + " li:eq(" + loadPrev + ")";
                    q.appendTo(n).wrap("<a " + g + o + "></a>").parent().parent().addClass("img-loading prevslide");
                    q.load(function () {
            ...
            var l = a('<img src="' + api.getField("img") + '"/>');
            var f = d.el + " li:eq(" + vars.current_slide + ")";
            l.appendTo(f).wrap("<a " + g + o + "></a>").parent().parent().addClass("img-loading activeslide");
            l.load(function () {
                d._origDim(a(this));
                d.resizeNow();
                d.launch();
            ...
            if (d.options.slides.length > 1) {
                ...
                var e = a('<img src="' + d.options.slides[loadNext].img + '"/>');
                var p = d.el + " li:eq(" + loadNext + ")";
                e.appendTo(p).wrap("<a " + g + o + "></a>").parent().parent().addClass("img-loading");
                e.load(function () {
                    a(this).data("origWidth", a(this).width()).data("origHeight", a(this).height());
                    d.resizeNow()
                })
        ...
        d.resizeNow = function () {
            return d.$el.each(function () {
                a("img", d.el).each(function () {
                    ...
                    if (thisSlide.parents("li").hasClass("img-loading")) {
                        a(".img-loading").removeClass("img-loading")
                    }
        ...
        d.nextSlide = function () {
            ...
            if (!a(k).html()) {
                ...
                imgLink = (d.options.slides[loadSlide].url) ? "href='" + d.options.slides[loadSlide].url + "'" : "";
                var f = a('<img src="' + d.options.slides[loadSlide].img + '"/>');
                f.appendTo(k).wrap("<a " + imgLink + j + "></a>").parent().parent().addClass("img-loading").css("visibility", "hidden");
                f.load(function () {
                    d._origDim(a(this));
                    d.resizeNow()
                })
            ...
            if (d.options.thumbnail_navigation == 1) {
                vars.current_slide - 1 < 0 ? prevThumb = d.options.slides.length - 1 : prevThumb = vars.current_slide - 1;
                a(vars.prev_thumb).html(a("<img/>").attr("src", d.options.slides[prevThumb].img));
                nextThumb = loadSlide;
                a(vars.next_thumb).html(a("<img/>").attr("src", d.options.slides[nextThumb].img))
            }
            if (typeof theme != "undefined" && typeof theme.beforeAnimation == "function") {
                theme.beforeAnimation("next")
            }
        ...
        d.goTo = function (f) {
            ...
            if (e - f > vars.current_slide) {
                ...
                vars.update_img = "next";
                d._placeSlide(vars.update_img)
            } else {
                if (e - f < vars.current_slide) {
                    vars.current_slide = e - f + 1;
                    vars.update_img = "prev";
                    d._placeSlide(vars.update_img)
                }
            }
            if (d.options.slide_links) {
        ...
        d._placeSlide = function (e) {
            ...
            if (e == "next") {
                ...
                if (!a(g).html()) {
                    ...
                    imgLink = (d.options.slides[loadSlide].url) ? "href='" + d.options.slides[loadSlide].url + "'" : "";
                    var f = a('<img src="' + d.options.slides[loadSlide].img + '"/>');
                    f.appendTo(g).wrap("<a " + imgLink + h + "></a>").parent().parent().addClass("img-loading").css("visibility", "hidden");
                    f.load(function () {
                        d._origDim(a(this));
                        d.resizeNow()
                    })
