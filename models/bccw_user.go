...
func (u User)Quereuser()(*User,error){
   ...
   err:= row.Scan(&u.Id,&u.UserName,&u.UserPassword,&u.UserPhone)
	if err != nil {
		return nil,err
	}
    return &u,nil
...
func (u User) Inseruser()(int64,error){
	result,err:=dbMysqlSV.Db.Exec("insert into bccw_user(user_name ,user_password,user_phone ) values(?,?,?) ",u.UserName,u.UserPassword,u.UserPhone)
	if err != nil {
